...
//!     明确一个观点：指针就是地址。没有任何指向之类的说法或描述，这些说法都是错的。
//!     一般经常把指针变量和其保存的指针搞混，人们容易把指针变量简称为指针，但说的人多了也成了规矩，所以有时指针指的是指针变量的简称，而非一个内存地址。
//!     产生这个迷惑点的原因在于，变量a是指针变量。a的值是一个指针，我们就会简称a是一个指针，那我们指的是a的值是一个指针，还是a这个变量是一个指针呢？
//!     他们的区别是：
//!             指针指代地址时，它仅是一个值，可以成为指针变量的变量值。
//!             指针指代指针变量时，它拥有变量自身的地址和变量值（也是一个地址）两个属性。
//! 智能指针变量：
//!     存放了指针，和一些长度等描述信息的变量。
//! &和*的关系：
//!     *a语义表示直接从a（的值）这个指针（内存地址）中取值，即[[0x00001111]]，可读可写。
//!     一般来说，&a也是一个指针，将&a存在变量b里，那么变量b就属于指针变量，其值为a的地址。
