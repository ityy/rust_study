...
/// 但这也带来了一些限制。在线程切换和跨线程共享数据上会产生很多额外开销。
/// 即使是一个什么都不做的线程也会用尽珍贵的系统资源，而这就是异步代码要减少的开销。
/// 我们可以使用 Rust 的 async/.await 重写上面的函数，
/// 实现执行多个任务的目标而不用创建多个线程：
///
/// ```no_run
///  async fn get_two_sites2() {
...
/// ```
///
///总之，相比多线程实现来说，异步实现的应用具有使用更少的资源获得更高性能的潜力。
/// 线程由操作系统支持，使用它们并不需要特别的编程模型——任何函数都可以创建一个线程，而调用一个使用多线程的函数就像调用一个普通函数一样简单。
/// 但是异步函数就需要语言层面或者类库层面提供特殊的支持才能工作。
