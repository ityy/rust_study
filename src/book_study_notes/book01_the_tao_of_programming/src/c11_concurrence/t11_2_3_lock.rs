...
fn test_not_deadlock() {
    ...
    for id in 0..runs {
        ...
        thread::spawn(move || {
            ...
            flip_simulate(target_flips, total_flips, id); // 先获取total_flips_lock
...
fn test_deadlock() {
    ...
    for id in 0..runs {
        ...
        thread::spawn(move || {
            println!("thread {} start", id);
            flip_simulate(target_flips, total_flips, id);
...
/// # 投掷硬币函数
fn flip_simulate(target_flips: u64, total_flips: Arc<Mutex<u64>>, id: u64) {
    ...
    println!("iter_counts(thread:{}): {}", id, iter_counts);
