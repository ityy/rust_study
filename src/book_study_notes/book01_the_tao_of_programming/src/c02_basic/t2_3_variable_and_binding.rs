...
#[test]
fn testBorrow() {
    //注意：所有变量标识都本身代表一个内存地址。代码中使用变量标识，默认都是使用这个标识所代表的地址中的值。
    //栈:
    //let [a]=3; a代表栈地址0x00000001 [a]为3
    //堆:
    //let [b]=String::new("hello"); b代表栈地址0x00000002；[b]为0x10000001，此为hello在堆中的地址。
    ...
    println!("a addr:{:p}", b); //{:p}表示打印指针值 0x55fdfeec4 b的值是a的地址，所以可以按指针打印。注意不是b的地址。
    println!("b addr:{:p}", &b);
