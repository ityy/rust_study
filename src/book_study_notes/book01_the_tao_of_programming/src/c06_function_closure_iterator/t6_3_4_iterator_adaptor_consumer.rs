//! # 迭代器的适配器与消费器
//! 迭代器提供了Map、Filter等适配器，这里有点类似java的流式处理，但Rust的实现更加具体，更能体会到本质。<br/>
...
/// # 消费器（Consumer）
/// 迭代器适配器都是对next的包装增强，没有实际调用next进行消费的行为。Rust中的迭代器都是惰性的。<br/>
/// 最简单的消费行为就是for循环，其会隐性的调用next方法。<br/>
...
/// -   collect 收集结果消费器     返回集合    消费迭代器，并将每一个元素收集到指定的集合类型中。也称为收集器。
#[test]
fn test_any_fold() {
    let a = [1, 2, 3];
