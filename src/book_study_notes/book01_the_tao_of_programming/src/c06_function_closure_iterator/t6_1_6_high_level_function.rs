//! # 高阶函数
//! 在数学中，高阶函数也叫算子或泛函。比如微积分中的导数就是一个函数到另一个函数的映射。<br/>
//! 在计算机科学里，高阶函数是指以函数作为参数或返回值的函数，它也是函数式编程语言最基础的特性。<br/>
//! 在Rust中，函数是一等公民。这表示函数也可以作为函数的参数和返回值使用。
...
pub fn function_is_variable() {
    //函数指针
    let sum_fn_pointer: fn(i32, i32) -> i32 = sum;//sum_fn_pointer为函数指针，其类型为fn(i32, i32) -> i32
    println!("{:p}", sum_fn_pointer);//能打印指针地址
