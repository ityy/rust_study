//! 闭包
//! 闭包通常指词法闭包，是一个持有外部环境变量的函数。
//! 外部环境是指闭包定义时所在的词法作用域。
//! 外部环境变量，在函数式编程中也被称为自由变量，是指并不在闭包中定义的变量。
//! 将自由变量和自身绑定的函数就是闭包。
...
fn counter(i: i32) -> Box<dyn Fn(i32) -> i32> {
    ...
    fn inc(s: i32) -> i32 {
        //s + i //error 函数无法使用外部变量,只能通过参数传进来
        s + 1 //只能固定写好
    }
    // 如果使用闭包，则可通过捕获环境变量生成闭包。
    // 注：闭包的大小编译器未知，所以用Box包装
...
fn test_principle() {
    ...
    /// 闭包本质是Rust提供的语法糖，和函数的区别就是可以捕获外部环境变量。
    /// 闭包由编译器来解析转换，也可以通过其它方法来实现闭包。
    ...
    /// 有了这三个抽象，实现闭包就简单了。我们用struct模拟闭包，用impl这三个trait定义行为。
    /// 每个闭包表达式，实际上都是该闭包结构体的示例，外部环境变量被捕获，放在了结构体的成员内。
...
/// 闭包与所有权
/// 前面提到，闭包表达式会由编译器自动翻译为结构体实例，并为其实现FnOnce、Fn、FnMut中的一个trait。
/// 如何知道具体实现了哪一个trait呢？这三个trait和所有权有关。
