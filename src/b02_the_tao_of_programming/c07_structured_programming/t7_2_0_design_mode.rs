...
/// 建造者模式
/// Rust最常用的模式之一。Rust结构体没有构造函数，不像java提供默认构造函数。也没有默认值，不像java会将值初始化为0。
...
/// 访问者模式
/// 访问者模式用于将数据结构和作用于结构上的操作解耦。即数据结构在calss或struct上，操作在interface或trait上。
/// 访问者模式一般包含两个层次：
///     定义需要操作的元素
///     定义相关的操作
///
/// Serde
/// 访问者模式的一个经典案例就是第三方库Serde，这是一个高效的序列化/反序列化框架，名字也是由Serialize和Deserialize的前几个字母拼接而成。
...
fn test_raii_2() {
    ...
    ///
    /// Rust语言的哲学是组合由于继承，结构体，枚举体，trait，就是一个个榫卯（sun mao），可以自由组合出想要的结构。
