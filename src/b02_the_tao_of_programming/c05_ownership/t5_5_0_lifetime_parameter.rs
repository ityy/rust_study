//! 生命周期参数
//! 值的生命周期与词法作用域有关，但引用可以在各个函数间传递，必然会跨越多个词法作用域。
//! 对于函数本地声明的拥有所有权的值或者借用来说，Rust编译器包含的借用检查器（borrow checker）可以检查它们的生命周期。
//! 但是对于跨词法作用域的引用，借用检查器就难以判断合法性了。
//! 我们需要手动标注生命周期参数，来帮助编译器进行借用检查，以便防止悬垂指针的出现。
//!
//! 显式生命周期参数
//! 生命周期参数以单引号开头，标识通常为小写字母，比如'a。
//! 标注生命周期参数示例：
//!     x:&i32          引用
//!     x:&'a i32       标注生命周期的引用
...
#[test]
fn test_fn_lifetime() {
    /// 限制：返回值的生命周期长度必须不能大于入参的生命周期长度
    /// 禁止在没有任何入参的情况下返回引用，因为这必然带来悬垂指针
    fn foo<'a>(s: &'a str, t: &'a str) -> &'a str { s }

    /// 泛型声明必须在生命周期参数的后面
    fn bar<'a, T>(s: &'a T, t: &'a T) -> &'a T { s }

    /// 也可以标注多个生命周期参数，且指明它们之间的大小关系
//    fn foobar<'a, 'b>(s: &'a str, t: &'b str) -> &'a str {
//        if s.len() > t.len() { s } else { t }
//    } //error[E0623]: lifetime mismatch  编译器不知道a b的大小关系
    // 'b: 'a 表示'b 比'a存活的更长。
    // 由于有返回值不能比入参生命周期长的限制。, 标记'b 比'a存活的更长是对的
...
fn test_struct_lifetime() {
    /// 限制：结构体的生命周期，应小于等于任意成员的生命周期。
    ///     否则成员先行析构，则结构体的引用就称为悬垂指针，这是编译器不允许的。
    #[derive(Debug)]
    struct Foo<'a> {
        part: &'a str,
    }
...
///     T:'a            T类型的任何引用都要获得和'a一样长
///     T:'a + Trait    T类型必须实现Trait，且任何引用都要获得和'a一样长
fn nothing() {}