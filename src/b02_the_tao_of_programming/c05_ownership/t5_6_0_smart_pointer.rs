...
fn test_rc() {
    ...
    let z1 = Rc::downgrade(&y1); //增加弱引用 弱引用没有所有权
...
/// 值语义：Cell<T> 引用语义：RefCell<T> 内部可变性
/// 当结构体本身不可变，但成员需要可变时怎么办
/// Rust提供了内部可变性
...
fn test_cell() {
    #[derive(Debug)]
    /// 值语义使用Cell
    /// 我们通过get/set方法对其操作，非常类似OOP语言中的getter/setter方法。
    /// 无运行时消耗
    struct Foo {
        x: u32,
        y: Cell<u32>,
...
fn test_refcell() {
    #[derive(Debug)]
    /// 引用语义使用RefCell
    /// 通过borrow_mut或borrow来获取引用
    /// 有运行时借用检查消耗，检查不通过会引起线程panic
    struct Foo {
        x: u32,
        y: RefCell<String>,
    }
...
/// Cow<T> 写时复制
/// 写时复制（copy on write）技术是一种程序优化的策略。
/// Cow<T>是一个枚举体的智能指针，有两个可选值：
///     Borrowed    用于包裹引用
///     Owned       用于包裹所有者
/// Cow<T>提供的功能是：以不可变的方式访问借用内容，在需要可变或所有权的情况下克隆一份数据。适合读多写少的场景。
...
fn test_cow() {
    ...
    //有可变需求 且i2没有s2的所有权 会克隆
    //注意:借用数据被克隆为了新的对象
    let s2 = [1, 2, 3, -45, 6];
    let mut i2 = Cow::from(&s2[..]);//Cow内只是s2的引用，没有所有权。所以当发生修改时，Cow会自动克隆一份数据。
    abs_all(&mut i2);
    println!("IN: {:?}", s2);
